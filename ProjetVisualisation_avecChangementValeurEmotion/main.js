...
d3.csv('donneesRTS.csv',function(d){
    ...
    //Mise en place des variables et événement pour le dialog permettant de changer la valeur émotion d'une nouvelle
    const choixEmotion = document.getElementById('choixEmotion')
    const selectEl = choixEmotion.querySelector('select')
    const confirmBtn = choixEmotion.querySelector('#confirmBtn')
    ...
    function resetDialog(){
        const defaultOption = choixEmotion.querySelector('#default')
    ...
    path
      ...
      .on('click', function(d){
        if(this.id == 'total'){
            ...
            resetDialog()
            choixEmotion.showModal()
            choixEmotion.addEventListener('close', () => {
                if(selectEl.value == 'Mauvaise'){
                    dataTotal['Bonnes nouvelles'] --
                    dataTotal['Mauvaises nouvelles'] ++
                    updateEmotion(dataTotal)
                }
                if(selectEl.value == 'Neutre'){
                    uneBonneNouvelle.emotion == 'Neutral'
                    dataTotal['Bonnes nouvelles'] --
                    dataTotal['Nouvelles neutres'] ++
                    updateEmotion(dataTotal)
                }
            })         
            }
